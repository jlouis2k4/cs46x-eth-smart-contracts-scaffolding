...
contract tollInfrastructure {
    ...
    function checkInfNameIsNotAlreadyInArray(string memory infName) private view returns(bool) {
    ...
    function updateUserType(individualUser memory currUser, infrastructure memory currInf) private view {
        for (uint i = 0; i < userStructArray.length; i++){
    ...
    function getUserCost(individualUser memory currUser, infrastructure memory currInf) private pure returns (uint userType) {
