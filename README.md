...
- [SKIP TO QUICKSTART GUIDE](https://github.com/KnoxSamuel/cs46x-eth-smart-contracts-scaffolding/main/README.md#scaffold-toolkit--framework)
...
# Scaffold Toolkit + Framework
Based on the [scaffold-eth-2 toolkit](https://github.com/scaffold-eth/scaffold-eth-2), an open-source, up-to-date all-in-one testing/deployment framework for building decentralized applications (dapps) on the Ethereum blockchain. It's designed to make it easier for developers to create and deploy smart contracts and build user interfaces that interact with those contracts. Documentation is also available online [here](https://docs.scaffoldeth.io/scaffold-eth/).

⚙️ Built using NextJS, RainbowKit, Hardhat, Wagmi, and Typescript.

- ✅ **Contract Hot Reload**: The frontend auto-adapts to the smart contracts as they change.
...
Once ready to deploy, there are a few things to adjust.

1. Select the network

By default, `yarn deploy` will deploy the contract to the local network. Change the defaultNetwork in `packages/hardhat/hardhat.config.ts.` Or simply run `yarn deploy --network target_network` to deploy to another network.

Check the `hardhat.config.ts` for pre-configured networks. You can also add other network settings to the `hardhat.config.ts file`. Here are the [Alchemy docs](https://docs.alchemy.com/docs/how-to-add-alchemy-rpc-endpoints-to-metamask) for information on specific networks.
...
2. Generate a new account or add one to deploy the contract(s) from. Additionally, you will need to add your Alchemy API key. Fill the required keys in `.env`.
...
The deployer account is the account that will deploy the contracts. Additionally, the deployer account will be used to execute any function calls that are part of the deployment scripts.

Either generate a random account / private key with `yarn generate`, or add the private key of your crypto wallet (probably one dedicated to testnet development so as to not commingle keys & information). `yarn generate` will create a random account and add the DEPLOYER_PRIVATE_KEY to the .env file. You can view the generated account with `yarn account`.

3. Deploy the smart contracts

Run the command below to deploy the smart contract to the target network. Make sure to have some funds in your deployer account to pay the gas fees.
...
To deploy directly from the CLI, run `yarn vercel` and follow the steps to deploy to Vercel. Once you log in (email, github, etc), the default options should work. It'll give you a public URL.
...
## Interacting with the Smart Contracts: Scaffold-ETH 2 Custom Hooks
...
Use this hook to subscribe (listen) to events emitted by your smart contract, and receive real-time updates when these events are emitted.
